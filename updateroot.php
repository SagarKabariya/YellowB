...
session_start();
 ...
 <h1>--------Manage Bus--------</h1></center>
	   ...
	   $er1=0;
	   $er2=0;
       $get=  mysql_query("select * from route where route_id=$_REQUEST[id]");
       $g= mysql_fetch_array($get);
        if(isset($_REQUEST["back"]))
        {
			header('location:showroot.php');
		}
        if(isset($_REQUEST['save']))
        {
			
            if($_REQUEST['scityname']=="")
            {
                $er=1;
            }
			
            if($_REQUEST['dcityname']=="")
            {
                $er=1;
            }
            ...
            if($er!=1 && $f!=1)
            {
				//echo $_REQUEST['scityname'],$_REQUEST['dcityname'],$_REQUEST['bus'],$_REQUEST['txtfare'];
                $in=  mysql_query("update route set `source_id`='$_REQUEST[scityname]',`destination_id`='$_REQUEST[dcityname]',`bus_id`='$_REQUEST[bus]',`fare`= '$_REQUEST[txtfare]' where route_id='$_REQUEST[id]'");
                header('location:showroot.php');
            }
		}   
  ?>
 ...
 <form method="post">
            <table  border="0" cellpadding="0" cellspacing="0" align="center" width="300">
                     <form action="" name="state" method="post">
                            <tr>
                             <td valign="top" align="center" colspan="2"><h3><b>Add Route</b></h3></td>
                 </tr>
                         <tr><td><br></td></tr>
			  ...
			  <td> Source City:</td>
			  <td>
 <select id="scityname" name="scityname" style="width:200px;">
          <option value="">-----Select City-----</option>
          <?php
		  			$resmain=mysql_query("select * from city");
			...
			<tr>
			  ...
			  <td>
				<select id="dcityname" name="dcityname" style="width:200px;">
					<option value="">-----Select City-----</option>
				<?php
		  			$resmain=mysql_query("select * from city");
					while($row=mysql_fetch_array($resmain))
            ...
            <tr>
			  <td>Select Bus:</td>
			  <td>
			  <select id="bus" name="bus" style="width:200px;">
					<option value="">-----Select Bus-----</option>
				<?php
		  			$resmain=mysql_query("select * from bus");
					while($row=mysql_fetch_array($resmain))
					{
			...
			<tr>
			  <td>Fare:</td>
			  <td><input type="text" id="txtfare" name="txtfare">	
              </td>
			</tr>
                         <tr><td><br></td></tr>
                         <tr align="center">
                             <td colspan="2"><input name="save" type="submit" class="button" value="submit">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp&nbsp
                                 <input  type="reset" class="button" value="Reset"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp&nbsp
                                  <input name="back" type="submit" class="button" value="Back to page"> </td>
                            </tr>                     
                     </form> 
                </table>
                <br/>
